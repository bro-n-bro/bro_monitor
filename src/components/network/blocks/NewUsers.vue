<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.blocks.newUsers'] }">
        ...
        <div class="val">
            <Loader v-if="!store.cache.new_accounts_actual" />
    ...
    onBeforeMount(async () => {
        ...
        if (!store.cache.new_accounts_actual) {
            try {
                fetch('https://rpc.bronbro.io/statistics/new_accounts/actual')
                    .then(res => res.json())
                    .then(response => store.cache.new_accounts_actual = response.data)
            } catch (error) {
                console.error(error)
            }
        }
        ...
        if (!store.cache.new_accounts) {
            try {
                ...
                // Request
                await fetch(`https://rpc.bronbro.io/statistics/new_accounts?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                    .then(res => res.json())
