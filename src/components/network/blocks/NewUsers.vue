<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.blocks.newUsers'] }">
        <div class="btns">
...
<script setup>
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_blocks_new_users_title')+ ': ' + series[0][dataPointIndex].toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(() => {
        // Get data
        if (!store.cache.new_accounts_actual) {
            try {
                fetch('https://rpc.bronbro.io/statistics/new_accounts/actual')
                    .then(res => res.json())
                    .then(response => {
                        // Set data
                        store.cache.new_accounts_actual = data.value = response.data
                    })
            } catch (error) {
                console.error(error)
            }
        ...
        if (!store.cache.new_accounts) {
            try {
                ...
                // Request
                fetch(`https://rpc.bronbro.io/statistics/new_accounts?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                    .then(res => res.json())
                    .then(response => {
                        store.cache.new_accounts = responseData.value = response.data
