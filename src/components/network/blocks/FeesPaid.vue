                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_blocks_fees_paid_title')+ ': ' + Number((series[0][dataPointIndex] / Math.pow(10, store.networks[store.currentNetwork].exponent)).toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(() => {
        ...
        if (!store.cache.fees_paid_actual) {
            try {
                fetch('https://rpc.bronbro.io/statistics/fees_paid/actual')
                    ...
                    .then(response => {
                        // Set data
                        store.cache.fees_paid_actual = data.value = response.data
                    })
            } catch (error) {
                console.error(error)
            }
        ...
        if (!store.cache.fees_paid) {
            try {
                ...
                // Request
                fetch(`https://rpc.bronbro.io/statistics/fees_paid?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                    .then(res => res.json())
                    .then(response => {
                        store.cache.fees_paid = responseData.value = response.data
