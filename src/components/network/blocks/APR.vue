<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.blocks.APR'] }">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.blocks.APR')">
                <svg><use xlink:href="@/assets/sprite.svg#ic_pin"></use></svg>
            </button>

            <router-link :to="`/${store.currentNetwork}/chart/APR`" class="btn">
...
<script setup>
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_blocks_APR_title')+ ': ' + (series[0][dataPointIndex] * 100).toFixed(2) + '%</div>' +
                                '</div>'
                    return html
                }
            },
    ...
    onBeforeMount(() => {
        // Get data
        if (!store.cache.apr_actual) {
            try {
                fetch('https://rpc.bronbro.io/statistics/apr/actual')
                    .then(res => res.json())
                    .then(response => {
                        // Set data
                        store.cache.apr_actual = data.value = response.data
                    })
            } catch (error) {
                console.error(error)
            }
        ...
        if (!store.cache.apr) {
            try {
                ...
                // Request
                fetch(`https://rpc.bronbro.io/statistics/apr?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                    .then(res => res.json())
                    .then(response => {
                        store.cache.apr = responseData.value = response.data
