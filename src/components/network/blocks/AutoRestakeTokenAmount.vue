<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.blocks.autoRestakeTokenAmount'] }">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.blocks.autoRestakeTokenAmount')">
                <svg><use xlink:href="@/assets/sprite.svg#ic_pin"></use></svg>
            </button>

            <router-link :to="`/${store.currentNetwork}/chart/auto_restake_token_amount`" class="btn">
                <svg><use xlink:href="@/assets/sprite.svg#ic_fullscreen"></use></svg>
            </router-link>
        </div>

        <div class="title">
            {{ $t('message.network_blocks_auto_restake_token_amount_title') }}
        </div>

        <div class="val">
            <Loader v-if="!store.cache.restake_token_amount_actual" />
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_charts_auto_restake_token_amount_title') + ': ' + Number((series[0][dataPointIndex] / Math.pow(10, store.networks[store.currentNetwork].exponent)).toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
                    return html
                }
            },
    ...
    onBeforeMount(async () => {
        ...
        if (!store.cache.restake_token_amount) {
            try {
                ...
                // Request
                await fetch(`https://rpc.bronbro.io/statistics/restake_token_amount?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                    .then(res => res.json())
