<template>
     <div class="block full_w" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.validatorGroupMap'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_validator_group_map_commission'))">
        <div class="btns">
...
<script setup>
    ...
    onBeforeMount(() => {
        store.cache.validatorsMap.forEach(el => {
            // Set chart data
            series.value.push({'data': [[el.commission * 100, el.delegators]]})
