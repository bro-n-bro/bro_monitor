<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.bondedRatio'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_bonded_ratio', { token: store.networks[store.currentNetwork].token_name }))">
        <div class="btns">
...
<script setup>
            ...
            custom: function({ dataPointIndex, w }) {
                let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                    ...
                    html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                ...
                                '<div class="tooltip_val">'+ i18n.global.t('message.network_charts_bonded_ratio_title', { token: store.networks[store.currentNetwork].token_name })+ ': ' + store.cache.charts.bonded_ratio[dataPointIndex].y.toFixed(2) + '%</div>' +
                            '</div>'
    ...
    function initChart() {
        ...
        // Set labels
        store.cache.charts.bonded_ratio.forEach(el => {
