<template>
     <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.inactiveAccounts'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_inactive_accounts'))">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.charts.inactiveAccounts')">
                <svg><use xlink:href="@/assets/sprite.svg#ic_pin"></use></svg>
            </button>

            <router-link :to="`/${store.currentNetwork}/chart/inactive_accounts`" class="btn">
                <svg><use xlink:href="@/assets/sprite.svg#ic_fullscreen"></use></svg>
            </router-link>
        </div>

        <div class="title">
            {{ $t('message.network_charts_inactive_accounts_title') }}
        </div>
...
<script setup>
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    '<div class="tooltip_date">' + store.cache.charts.inactive_accounts[dataPointIndex].x + '</div>' +
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_blocks_inactive_accounts_title') + ': ' + store.cache.charts.inactive_accounts[dataPointIndex].y.toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        // Get chart data
        if (!store.cache.charts.inactive_accounts) {
            await getChartData()
        }
    ...
    async function getChartData() {
        // Request
        await fetch(`https://rpc.bronbro.io/statistics/inactive_accounts_historical?from_date=${store.currentTimeRangeDates[0]}&to_date=${store.currentTimeRangeDates[1]}&detailing=${store.currentTimeRangeDetailing}`)
            .then(res => res.json())
            .then(response => store.cache.charts.inactive_accounts = response.data)
    }
    ...
    function initChart() {
        ...
        // Set labels
        store.cache.charts.inactive_accounts.forEach(el => {
