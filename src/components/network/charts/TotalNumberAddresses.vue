<template>
     <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.totalNumberAddresses'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_addresses'))">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.charts.totalNumberAddresses')">
        ...
        <div class="title">
            {{ $t('message.network_charts_total_number_addresses_title') }}
        </div>
...
<script setup>
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_charts_total_number_addresses_title') + ': ' + Number(store.cache.charts.total_accounts[dataPointIndex].y.toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    function initChart() {
        ...
        // Set labels
        store.cache.charts.total_accounts.forEach(el => {
