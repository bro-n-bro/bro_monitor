<template>
     <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.newAccounts'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_new_accounts'))">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.charts.newAccounts')">
        ...
        <div class="title">
            {{ $t('message.network_charts_new_accounts_title') }}
        </div>
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_charts_new_accounts_title') + ': ' + store.cache.charts.new_accounts[dataPointIndex].y.toLocaleString('ru-RU') + '</div>' +
                                '</div>'
