<template>
     <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.gasPaid'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_gas_paid'))">
        <div class="btns">
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_charts_gas_paid_title') + ': ' + Number(store.cache.charts.gas_paid[dataPointIndex].y.toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
