<template>
    <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.undondedToken'] }" @mouseover="emitter.emit('setNotification', $t('message.notice_undonded_token', { token: store.networks[store.currentNetwork].token_name }))">
        <div class="btns">
...
<script setup>
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    '<div class="tooltip_date">' + store.cache.charts.unbonded_tokens[dataPointIndex].x + '</div>' +
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_charts_undonded_token_title', { token: store.networks[store.currentNetwork].token_name }) + ': ' + Number((store.cache.charts.unbonded_tokens[dataPointIndex].y / Math.pow(10, store.networks[store.currentNetwork].exponent)).toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        // Get chart data
        if (!store.cache.charts.unbonded_tokens) {
            await getChartData()
        }
    ...
    async function getChartData() {
        // Request
        await fetch(`https://rpc.bronbro.io/statistics/unbonded_tokens?from_date=${store.currentTimeRangeDates[0]}&to_date=${store.currentTimeRangeDates[1]}&detailing=${store.currentTimeRangeDetailing}`)
            .then(res => res.json())
            .then(response => store.cache.charts.unbonded_tokens = response.data)
    }
    ...
    function initChart() {
        // Set chart data
        store.cache.charts.unbonded_tokens.forEach(el => chartData.value.push(el.y))
        ...
        // Set colors
        chartColors.value.push(store.cache.charts.unbonded_tokens[store.cache.charts.unbonded_tokens.length - 1].y >= Math.max(...chartData.value) ? '#1BC562' : '#EB5757')

        // Set labels
        store.cache.charts.unbonded_tokens.forEach(el => {
