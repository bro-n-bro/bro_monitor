<template>
     <div class="block" :class="{ pinned: store.pinnedBlocks['cosmoshub.charts.activeUsers'] }">
        <div class="btns">
            <button class="pin_btn btn" @click.prevent="emitter.emit('togglePinBlock', 'cosmoshub.charts.activeUsers')">
                <svg><use xlink:href="@/assets/sprite.svg#ic_pin"></use></svg>
            </button>

            <router-link :to="`/${store.currentNetwork}/chart/active_users`" class="btn">
                <svg><use xlink:href="@/assets/sprite.svg#ic_fullscreen"></use></svg>
            </router-link>
        </div>

        <div class="title">
            {{ $t('message.network_charts_active_users_title') }}
        </div>
...
<script setup>
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_charts_active_users_title')+ ': ' + Number(series[0][dataPointIndex].toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    function initChart() {
        ...
        // Set labels
        store.cache.active_accounts.forEach(el => {
