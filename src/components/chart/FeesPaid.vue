<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_fees_paid'))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.fees_paid),
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">'+ i18n.global.t('message.network_blocks_fees_paid_title') + ': ' + Number((responseData.value[dataPointIndex].н / Math.pow(10, store.networks[store.currentNetwork].exponent)).toFixed(0)).toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.fees_paid !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/fees_paid?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                ...
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.fees_paid = response.data
