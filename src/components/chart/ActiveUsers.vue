...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.active_accounts),
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.active_accounts !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/active_accounts?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                ...
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.active_accounts = response.data
