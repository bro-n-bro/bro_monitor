<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_bonded_ratio', { token: store.networks[store.currentNetwork].token_name }))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.bonded_ratio),
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_blocks_bonded_ratio_title') + ': ' + responseData.value[dataPointIndex].y.toFixed(2) + '%</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.bonded_ratio !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            // Request
            await fetch(`https://rpc.bronbro.io/statistics/bonded_ratio?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                .then(res => res.json())
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.bonded_ratio = response.data
