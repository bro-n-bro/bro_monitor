<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_active_accounts'))">
        <Loader v-if="loading" />
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_blocks_active_accounts_title') + ': ' + responseData.value[dataPointIndex].y.toLocaleString('ru-RU') + '</div>' +
                                '</div>'
