<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_inactive_accounts'))">
        <Loader v-if="loading" />
...
<script setup>
    import { inject, ref, reactive, onBeforeMount, computed, onMounted } from 'vue'
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.inactive_accounts),
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_blocks_inactive_accounts_title') + ': ' + responseData.value[dataPointIndex].y.toLocaleString('ru-RU') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.inactive_accounts !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    onMounted(() => {
        // Disable some timeframes
        document.querySelector('.time_range .btn:nth-child(1)').classList.add('disabled')
        document.querySelector('.time_range .btn:nth-child(2)').classList.add('disabled')
    })
    ...
    async function getChartData(cacheEnable = true) {
        try {
            // Request
            await fetch(`https://rpc.bronbro.io/statistics/inactive_accounts_historical?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                .then(res => res.json())
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.inactive_accounts = response.data
