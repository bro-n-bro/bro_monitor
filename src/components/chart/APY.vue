...
<script setup>
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    '<div class="tooltip_date">' + store.cache.apy.value[dataPointIndex].x + '</div>' +
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_charts_APY_title') + ': ' + (series[0][dataPointIndex] * 100).toFixed(2) + '%</div>' +
                                '</div>'
    ...
    watch(() => store.updateTimeRangeDates, async () => {
        ...
        if (store.cache.apy && cache.value) {
            // Init chart
            initChart()
        } else {
            // Get chart data
