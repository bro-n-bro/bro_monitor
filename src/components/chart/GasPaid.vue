<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_gas_paid'))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.gas_paid),
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.gas_paid !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
