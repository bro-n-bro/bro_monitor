<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_undonded_token', { token: store.networks[store.currentNetwork].token_name }))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.unbonded_tokens),
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.unbonded_tokens !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
