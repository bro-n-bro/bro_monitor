<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_auto_restake_executed_count'))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.restake_execution_count),
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.restake_execution_count !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/restake_execution_count?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                ...
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.restake_execution_count = response.data
