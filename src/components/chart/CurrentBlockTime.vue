<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_current_block_time'))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.current_block_time),
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_charts_current_block_time_title') + ': ' + responseData.value[dataPointIndex].y.toFixed(2) + ' ' + i18n.global.t('message.network_charts_unit_sec') + '</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.current_block_time !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            // Request
            await fetch(`https://rpc.bronbro.io/statistics/blocks?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                .then(res => res.json())
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.current_block_time = response.data
