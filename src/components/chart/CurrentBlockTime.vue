...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.current_block_time),
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.current_block_time !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/blocks?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                ...
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.current_block_time = response.data
