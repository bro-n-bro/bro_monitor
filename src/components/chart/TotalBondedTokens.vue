...
<script setup>
                ...
                custom: function({series, seriesIndex, dataPointIndex, w}) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    '<div class="tooltip_date">' + store.cache.bonded_tokens_actual[dataPointIndex].x + '</div>' +
    ...
    watch(() => store.updateTimeRangeDates, async () => {
        ...
        if (store.cache.bonded_tokens_actual && cache.value) {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData () {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/apr?from_date=${from_date}&to_date=${to_date}&detailing=${detailing}`)
                .then(res => res.json())
                .then(response => store.cache.bonded_tokens_actual = response.data)
        } catch (error) {
            console.error(error)
        }
    ...
    function initChart() {
        // Set chart data
        store.cache.bonded_tokens_actual.forEach(el => chartData.value.push(el.y))
        ...
        // Set colors
        chartColors.value.push(store.cache.bonded_tokens_actual[store.cache.bonded_tokens_actual.length - 1].y >= Math.max(...chartData.value) ? '#1BC562' : '#EB5757')

        // Set labels
        store.cache.bonded_tokens_actual.forEach(el => {
