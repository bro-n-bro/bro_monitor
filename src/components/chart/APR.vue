<template>
    <div class="chart big" @mouseover="emitter.emit('setNotification', $t('message.notice_APR'))">
        <Loader v-if="loading" />
...
<script setup>
    ...
    const store = useGlobalStore(),
        ...
        responseData = ref(store.cache.charts.apr),
                ...
                custom: function({ dataPointIndex, w }) {
                    let left = w.globals.seriesXvalues[0][dataPointIndex] + w.globals.translateX,
                        ...
                        html = '<div class="chart_tooltip" style="'+ `left: ${left}px; top: ${top}px;` +'">' +
                                    ...
                                    '<div class="tooltip_val">' + i18n.global.t('message.network_charts_APR_title') + ': ' + (responseData.value[dataPointIndex].y * 100).toFixed(2) + '%</div>' +
                                '</div>'
    ...
    onBeforeMount(async () => {
        if (typeof store.cache.charts.apr !== 'undefined') {
            // Init chart
            initChart()
        } else {
            // Get chart data
    ...
    async function getChartData(cacheEnable = true) {
        try {
            ...
            await fetch(`https://rpc.bronbro.io/statistics/apr?from_date=${from_date.value}&to_date=${to_date.value}&detailing=${detailing.value}`)
                ...
                .then(response => {
                    cacheEnable
                        ? responseData.value = store.cache.charts.apr = response.data
